...
eightMeV_inJ = 1.2817418400000072 * 10**(-12)
sevenMeV_inJ = 1.1215241100000063 * 10**(-12)
sixMeV_inJ = 9.613063800000053 * 10 **(-13)
twoMeV_inJ = 3.204354600000018 * 10**(-13) # clipping
uz = ( c / (sevenMeV_inJ/(m_p*(c**2)) +1 ) ) * np.sqrt( ( sevenMeV_inJ/(m_p*(c**2)) + 1 )**2 - 1.0 ) # to get uz s.t. KE becomes equal to the variable named xMeV_inJ
...
print(f'For uz = {uz} m/s we get the below KE')
print(f'Initial Kinetic Energy is {KE_init_MeV} MeV')
...
while(counter <= nmax):
    ...
    if (counter%1000 == 0):
        print (f'counter value = {counter}') # to see some progress
...
np.savetxt("results_RKF4values_from6odesadaptiveRKF45.txt", np.array(results))
...
src = open("results_RKF4values_from6odesadaptiveRKF45.txt", "r") 
...
src.close() 
src = open("results_RKF4values_from6odesadaptiveRKF45.txt","w") 
...
line_count = get_no_of_lines_of_file("results_RKF4values_from6odesadaptiveRKF45.txt")

r_at_exit = np.loadtxt("results_RKF4values_from6odesadaptiveRKF45.txt", usecols=np.arange(0,3), skiprows=line_count-1) # get x,y,z coords at end of RK45 integration 
us_at_exit = np.loadtxt("results_RKF4values_from6odesadaptiveRKF45.txt", usecols=np.arange(3,6), skiprows=line_count-1) # get ux, uy, uz at end of RK45 integration
...
with open("results_RKF4values_from6odesadaptiveRKF45.txt" , "r") as f:
...
plt.savefig("2Dplot_zyplane_EBalongy_uinitalongz.pdf", bbox_inches='tight')
...
plt.ylabel("x [meters]")
plt.title("E and B along positive y. incident velocity along positive z")
plt.savefig("2Dplot_zxplane_EBalongy_uinitalongz.pdf", bbox_inches='tight')
...
with open("r_at_end.txt", "w") as f:
    np.savetxt(f, r_at_end, newline=" ")
# want to plot particle on the detector screen
# so on a (x-y) plane at a z_end
plt.figure()
plt.scatter(r_at_end[0], r_at_end[1], color='red') # r_at_end[0] is x at end, r_at_end[1] is y at end
plt.xlabel("x [meters]")
plt.ylabel("y [meters]")
plt.title("incident position on detector screen")
plt.show()
plt.savefig("xyscatter_at_detector_screen.pdf", bbox_inches='tight')
